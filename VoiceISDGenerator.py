from VoiceISDGeneratorClass import *
import sqlite3
import asyncio
...
now = datetime.now()
import random
my_object = VoiceISDGeneratorClass()
...
async def main():
    ...
    for row in results:
        device_id = row[0]
        print(device_id)
        result = await my_object.consume_voiceisd(device_id,access_token)
        #print(result)
        time.sleep(2)
        sums = [all_results[i] + result[i] for i in range(len(all_results))]
        all_results=sums
    print(all_results)
    f = open('/home/ec2-user/latency-test/FINAL_SCRIPTS/VoiceisdGenerator.log', 'a')
    f.write( 'Voiceisd, ' + datetime.now().strftime("%d/%m/%Y %H:%M:%S")+ ' , ' + str(all_results) + "\n")
    f.close()
