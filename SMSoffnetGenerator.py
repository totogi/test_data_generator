...
mnpinfo = config.mnpinfo
offnet = config.offnet
sms_session_sr = config.sms_session_sr
sms_session_er = config.sms_session_er
sms_session_in = config.sms_session_in
...
class EventGeneratorClass:
                ...
                payload = {
                        ...
                        "multipleUnitUsage": [
                                ...
                                "iMSChargingInformation": {
                                   ...
                                   "carrierSelectRoutingInformation": routeinfo,
                                    "numberPortabilityRoutingInformation": offnet
                                 },
                                        ...
                                        "usedUnitContainer": [
                                                {
                                                        ...
                                                        "serviceSpecificUnits": up_consumed_units
                                                }
                                        ],
                                        "ratingGroup": 200
                                }
        ...
        async def consume_data(self,device_id):
                for i in range(1):
                        ...
                        init_requ_units=random.randrange(sms_session_sr,sms_session_er,sms_session_in)
                        #print("init_requ_units " + str(init_requ_units))
                        up_consumed_units=random.randrange(sms_session_sr,sms_session_er,sms_session_in)
                        ...
                        #print("up_consumed_units " + str(up_consumed_units))
                        update_requ_units=random.randrange(sms_session_sr,sms_session_er,sms_session_in)
                        #print("Up requ units " + str(update_requ_units))
                        tr_consumed_units=random.randrange(sms_session_sr,sms_session_er,sms_session_in)
                        ...
                        #assign called number
                        called_number = random.randint(called_num_sr,called_num_er)
...
async def main():
    ...
    hrdata = conn.cursor()
    hrdata.execute("SELECT SMS_Offnet  FROM call_stats  where HOUR=?",[hrnow])
    ...
    for row in results:
        ...
        all_results=sums
    #print(all_results)
    stats_logger.info('VoiceOnnet, %s',str(all_results))
