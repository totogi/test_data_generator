from  DataGeneratorClass import *
import sqlite3
import asyncio
...
async def main():
    ...
    for row in results:
        device_id = row[0]
        print(device_id)
        result = await my_object.consume_data(device_id,access_token)
        #print(result)
        time.sleep(2)
        sums = [all_results[i] + result[i] for i in range(len(all_results))]
        all_results=sums
    print(all_results)
    f = open('/home/ec2-user/latency-test/FINAL_SCRIPTS/DataGenerator.log', 'a')
    f.write( 'Data, ' + datetime.now().strftime("%d/%m/%Y %H:%M:%S")+ ' , ' + str(all_results) + "\n")
    f.close()
